...
function Topbar() {
    ...
    const currentLevel =
        Math.floor(0.07 * Math.sqrt(user['currentXp'])) === 0
            ? 1
            : Math.floor(0.07 * Math.sqrt(user['currentXp']));
    const [xpAtNextLevel, setXpAtNextLevel] = useState(
        Math.floor(((currentLevel + 1) / 0.07) ** 2)
    );
    const [currentLevelXp, setCurrentLevelXp] = useState(Math.floor((currentLevel / 0.07) ** 2));
    ...
    useEffect(() => {
        ...
        if (level === user.level) {
            ...
            alert(
                `Level up! You'll be getting ${Math.round(
                    ((user.level * 50) / 5) ** 1.1
                )} coins now!`
            );
        ...
        <View style={styles.header}>
            <View style={styles.levels}>
                <View style={styles.level}>
                    <Text style={styles.levelText} adjustsFontSizeToFit={true} numberOfLines={1}>
                        {level}
                    </Text>
                </View>
                {progressBar}
